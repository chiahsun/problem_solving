...
struct Constaint {
    ...
    void print() const {
        printf("  Constraint : ");
        for (std::vector<std::pair<int, int> >::const_iterator it = points().begin(); it != points().end(); ++it)
            printf(" (%d, %d, %d)", it->first, it->second, board[it->first][it->second]);
        printf(" free -> ");
        int free = get_free();
        for (int i = 1; i <= 9; ++i)
...
#define FOR_EACH(it, container) for (std::vector<Constaint*>::iterator it = container.begin(); it != container.end(); ++it)

void board_set(int x, int y, int value) {
    assert(not board[x][y]);
    assert(value >= 1 and value <= 9);
    board[x][y] = value;
    FOR_EACH(it, board_constraints[x][y]) {
        assert(not (*it)->is_used(value));
        (*it)->set_used(value);
    }
}
...
void board_unset(int x, int y) {
    ...
    FOR_EACH(it, board_constraints[x][y]) {
        assert((*it)->is_used(old_value));
        (*it)->unset_used(old_value);
    }
}
...
bool search() {
    ...
    for (int i = 0; n_min_free > 1 and i < M; ++i)
        for (int k = 0; n_min_free > 1 and k < M; ++k) {
            if (not board[i][k]) {
                int free = ALL_FREE;
                FOR_EACH(it, board_constraints[i][k]) {
                    free &= (*it)->get_free();
                    if (free == 0) break;
                }
