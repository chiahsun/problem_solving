    ...
    while (scanf("%d", &nBuilding) == 1 && nBuilding) {
        ...
        for (int posBuilding = 0; posBuilding < nBuilding; ++posBuilding) {
            ...
            for (int posXId = cur.getBeginXId(); posXId < cur.getEndXId(); ++posXId) {
                const std::vector<BuildingId> & oldBuildingss = southernElevation[posXId];
                std::vector<BuildingId> newBuildings;

                bool isObsctructed = false;

                for (int posOldBuilding = 0; posOldBuilding < oldBuildingss.size(); ++posOldBuilding) {
                    BuildingId oldBuildingId = oldBuildingss[posOldBuilding];
                    const Building & oldBuilding = buildings[oldBuildingss[posOldBuilding]];
                    if (cur.getY() < oldBuilding.getY()) {
                        if (oldBuilding.getHeight() > cur.getHeight()) {
                            newBuildings.push_back(oldBuildingId);
                        } 
                    } else {
                        newBuildings.push_back(oldBuildingId);
                        if (cur.getHeight() <= oldBuilding.getHeight()) {
                            isObsctructed = true;
                            break;
                        }
                    }
                }
                if (not isObsctructed) {
                    newBuildings.push_back(posBuilding);
                    southernElevation[posXId] = newBuildings;
                }
            }
        }
