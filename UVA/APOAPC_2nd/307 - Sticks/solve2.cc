...
bool search(int left_length, int max_existed_stick_pos, int bound_stick_pos, std::vector<Stick> & vecStick, int nPicked, const int & stick_length) {
    if (debug) {
    ...
    int start = (left_length == stick_length) ? max_existed_stick_pos : bound_stick_pos;
    for (int i = start; i < vecStick.size(); ++i) {
        int cur_stick_length = vecStick[i].getLength();
        if (vecStick[i].size() != 0 and cur_stick_length <= left_length) {
            --vecStick[i].size();
            int new_max_existed_stick_pos = max_existed_stick_pos;
            if (vecStick[i].size() == 0) {
                while (new_max_existed_stick_pos < vecStick.size() and vecStick[new_max_existed_stick_pos].size() == 0)
                    ++new_max_existed_stick_pos;
            }
            bool ok = search(left_length-cur_stick_length, new_max_existed_stick_pos, i, vecStick, nPicked+1, stick_length);
            ++vecStick[i].size();
            if (ok) 
                return true;
